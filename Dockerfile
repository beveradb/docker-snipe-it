...
RUN \
 ...
 if [ -z ${SNIPEIT_RELEASE+x} ]; then \
	SNIPEIT_RELEASE=$(curl -sX GET "https://api.github.com/repos/beveradb/snipe-it/releases/latest" \
	| awk '/tag_name/{print $4;exit}' FS='[""]'); \
 fi && \
 curl -o \
 /tmp/snipeit.tar.gz -L \
	"https://github.com/beveradb/snipe-it/archive/${SNIPEIT_RELEASE}.tar.gz" && \
 tar xf \
